...
def create_dtst():
	...
	sensor_active = np.random.random((1300,4))*.2 + .75
	inps[200:1500,1] = sensor_noise[:,0]
	inps[200:1500,3:] = sensor_noise[:,1:]
	inps[200:1500,0] = sensor_active[:,0]
	inps[200:1500,2] = sensor_active[:,1]
	...
	#make the fake 1+3 sensor ON, turn left
	sensor_noise = (((np.random.random((1300,4))*2)-1.)*.3) - .2
	sensor_active = np.random.random((1300,4))*.2+.8
	inps[1700:,1:4] = sensor_noise[:,1:]
	inps[1700:,5] = sensor_noise[:,0]
	inps[1700:,0] = sensor_active[:,0]
	inps[1700:,4] = sensor_active[:,1]
...
if __name__=="__main__":
	inps, outs = create_dtst()
	import pickle
	f = open("taining_set.pickle","wb")
	pickle.dump([inps, outs],f)
	f.close()
